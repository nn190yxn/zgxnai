...
['media', 'banners', 'articles', 'pain-points', 'training', 'nutrition', 'membership', 'users', 'support'].forEach((id) => assert.ok(moduleSource.includes(id), `后台模块应包含 ${id}`));
...
['/media', '/banners', '/articles', '/pain-points', '/content/', '/membership/config', '/users/operations', '/support/tickets'].forEach((route) => assert.ok(moduleSource.includes(route), `运营模块应包含请求路径 ${route}`));
...
['media:read', 'banner:read', 'banner:write', 'article:read', 'pain_point:read', 'membership:read', 'user:read', 'ticket:read'].forEach((permission) => assert.ok(moduleSource.includes(permission), `运营模块应显示权限 ${permission}`));
