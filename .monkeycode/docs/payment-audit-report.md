...
> 当前状态说明（2026-08-21）：本文保留 2026-06-11 的历史问题记录。生产 MySQL 服务当前已将标准微信支付回调改为行锁事务处理，AES-GCM 回调解密先 Base64 解码并分离末尾 16 字节认证标签，平台证书支持配置文件或微信支付平台证书接口获取；小程序会员购买使用微信官方虚拟支付，支付完成后主动查询订单状态。当前自动化验证覆盖虚拟支付合规、订单幂等实现和回调静态链路。真实平台回调、证书轮换和生产配置仍需在微信及生产环境验收。

## 当前发布前结论

- [x] 虚拟支付会员方案使用 `wx.requestVirtualPayment`。
- [x] 普通微信支付接口拒绝虚拟会员套餐。
- [x] 虚拟支付发货通知使用事务、订单行锁和已支付状态幂等保护。
- [x] 标准微信支付回调使用事务、订单行锁和已支付状态幂等保护。
- [x] 回调密文按微信支付 AES-GCM 规范解密。
- [x] 订单号包含时间和密码学随机值，数据库 `order_no` 具有唯一约束。
- [x] 生产环境缺少虚拟支付消息推送 token 时拒绝消息推送请求。
- [ ] 生产环境真实证书、虚拟支付配置、平台回调和真机购买仍需外部环境验收。
