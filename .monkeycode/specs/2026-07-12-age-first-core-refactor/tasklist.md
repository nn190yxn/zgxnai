...
- [x] 11. 扩展年龄段痛点场景库
  - [x] 11.1 建立固定 4 类痛点类别
    - 在 `miniprogram/utils/core-action-scenes.js` 中为每个年龄段生成专注学习、情绪规则、运动体能、社交表达 4 个固定类别
    - 覆盖需求 R2.8，设计 Pain Point Category
  - [x] 11.2 扩展每类 10 个场景情况
    - 每个年龄段每个类别提供 10 个场景情况，合计每个年龄段 40 个、全量 280 个
    - 保留首批高频痛点作为首屏推荐场景
    - 覆盖需求 R2.9，设计正确性属性 P1-P3
  - [x]* 11.3 补充场景库数量和字段完整性测试
    - 扩展 `scripts/test-core-action-scenes.js`，断言每段 4 类、每类 10 个、字段完整
    - 覆盖设计正确性属性 P1、P2

- [x] 12. 调整首页痛点展示承载
  - [x] 12.1 首页首屏只展示 5 个高频痛点
    - 修改 `miniprogram/pages/index/index.js` 和 `index.wxml`，新增 `featuredPainPoints`，控制首屏展示数量
    - 覆盖需求 R1.5，设计正确性属性 P8
  - [x] 12.2 增加查看更多完整分组入口
    - 新增展开/收起交互，展开后按 4 个类别展示完整 40 个场景
    - 覆盖需求 R1.6、R2.8-R2.9，设计 Home Page State Machine
  - [x]* 12.3 补充首页展示策略测试
    - 扩展 `scripts/test-home-core-flow.js` 和 `scripts/test-miniprogram-core-ui-structure.js`，断言首屏 5 个和完整分组可展开
    - 覆盖设计正确性属性 P8
