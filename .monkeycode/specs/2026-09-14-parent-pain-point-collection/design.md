...
- 生产文章语料包含书籍章节与拆条内容（分类如 `认知健康`、`情绪养育`、`家庭教育`，标题形如「第X章 …」「第N步 …」「…（片段N）」）。已对匹配 `片段|第[0-9]+步|第[0-9一二三四五六七八九十百]+章` 的标题做排除（`isPainPointEligible` + `buildPainPointExclusion`），生产 2800 篇中排除 964 篇，抽样确认无误伤正常文章。序号只匹配阿拉伯数字，「第一步」「迈出社交第一步」这类正常标题保持可用。
- 仍有少量导入内容未被覆盖：标题形如「01 学习 越动越多的脑细胞」「11 该怎么正确地给孩子刷牙?」的编号条目，不含上述三种模式。编号开头规则会误伤正常问答标题，因此未纳入，需内容侧治理。
- `called_no_response` 这类痛点在生产语料里真实可用文章很少（收录期约 1 篇），合集内会呈现空状态；这反映内容供给，不是标签规则缺陷。
