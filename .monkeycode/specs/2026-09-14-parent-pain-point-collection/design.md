...
- 生产文章表混合了内容团队自产的方案卡/问答与约 1600 篇书籍导入内容（分类如 `家庭教育`、`大脑发育`、`认知健康`、`情绪养育`，标题形如「第X章 …」「…（片段N）」「… - …」「65 摇晃宝宝」「概念：解释」，作者为书名或外籍作者）。仅靠标题形态无法覆盖全部导入内容（如「运动对女性大脑健康的特殊价值」不含任何可判别形态），因此合集的来源判据改为「作者白名单」：只有 `CURATED_AUTHORS`（`小牛育儿内容组`、`小牛育儿编辑部`、`追光小牛`）或空白作者的署名才进入合集（`isPainPointEligible` + `buildPainPointExclusion`）。标题形态与分类排除保留为纵深防御。生产 2800 篇中，该判据把合集收录从约 1600 条降到 449 条，全部为内容团队自产内容。
- 副作用：内容团队自产语料的覆盖面有限，部分合集条目偏少（`body_adaptation` 10 条、`unclear_speech` 28 条、`cannot_play_together` 51 条），`called_no_response` 为 0 条并呈现空状态。这是内容供给问题：此前合集的「丰富」由书稿散文填充，改判据后暴露出真实供给量。补齐需要内容侧新增文章。
- 排除只作用于带 `pain_point_key` 的合集查询；常规文章列表与搜索仍会展示全部 `is_published=1` 的导入书稿，属独立的内容治理议题。
- 空白作者被当作可信来源（历史问答署名方式）。若未来导入内容遗漏作者字段，会被误纳入合集，需在导入流程侧保证作者字段正确。
