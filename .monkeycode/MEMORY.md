...
[用户协作节奏]
- Date: 2026-09-03
- Context: 本次小程序视觉与 Banner 管理开发
- Category: 工作流协作
- Instructions:
  - 有明确后续步骤时继续推进并完成验证；遇到会改变实现方向的关键不确定性时暂停并请求澄清。
...
- Instructions:
  ...
  - 生产目录不是 Git 工作树，热修复采用 SCP 同步 → pm2 restart；同步 `backend/src/mysql-production/server.js` 时需同时确认同目录 helper 依赖（如 `chat-context.js`、`daily-plan-text.js`、`development-zones.js`）和 `backend/examples/development-zones-3-6.json` 已在生产存在
...
- Instructions:
  - 回复优先采用自然对话式短句
  - 向用户说明产品方案和开发进展时使用普通语言，并解释必要的技术名词
